...
#include "device_buffer_gpu.h"
#include "gpu_error_check.h"

__global__
void computeAccKernel(
    ...
    double G)
{
    ...
    for (int j = 0; j < N; ++j)
    {
        ...
        double rx = px[j] - xi;
        double ry = py[j] - yi;
        double d2 = rx*rx + ry*ry + eps2;
        double invDist  = rsqrt(d2);
        double invDist3 = invDist * invDist * invDist;

        double s = G * mass[j] * invDist3;
...
void computeAccelerationsGPU_Optimized(
    const std::vector<Body>& bodies,
    std::vector<Vec>& acc,
    DeviceBuffers& buffers, // Pass persistent buffers
    double eps,
    double G)
{
    int N = bodies.size();
    ...
    // 1. Flatten host data for transfer
    // (Note: In a production engine, you would keep data structurally of arrays (SoA) to avoid this copy)
    static std::vector<double> h_px(MAX_BODIES), h_py(MAX_BODIES), h_mass(MAX_BODIES);
    
    // Only copy active bodies
    for (int i = 0; i < N; i++) {
    ...
    // 2. Copy Host -> Device (Only actual size N)
    size_t copySize = N * sizeof(double);
    gpuErrchk(cudaMemcpy(buffers.d_px, h_px.data(), copySize, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(buffers.d_py, h_py.data(), copySize, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(buffers.d_mass, h_mass.data(), copySize, cudaMemcpyHostToDevice));

    // 3. Launch Kernel
    int block = 256;
    int grid = (N + block - 1) / block;
    computeAccKernel<<<grid, block>>>(
        buffers.d_px, buffers.d_py, buffers.d_mass,
        buffers.d_ax, buffers.d_ay,
        N, eps2, G
    );
    
    // 4. Copy Device -> Host
    static std::vector<double> h_ax(MAX_BODIES), h_ay(MAX_BODIES);
    gpuErrchk(cudaMemcpy(h_ax.data(), buffers.d_ax, copySize, cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(h_ay.data(), buffers.d_ay, copySize, cudaMemcpyDeviceToHost));

    // 5. Update Acceleration Vector
    for (int i = 0; i < N; i++) {
        acc[i].x = h_ax[i];
        acc[i].y = h_ay[i];
    }
    ...
    DeviceBuffers& gpuBuffers)
{
    ...
    computeAccelerationsGPU_Optimized(
                    bodies, acc, gpuBuffers, 
                    eps, UniversalConstants::G_scaled
                );
...
int main()
{
    ...
    computeAccelerationsGPU_Optimized(
                    bodies, acc, gpuBuffers, 
                    softeningLength, UniversalConstants::G_scaled
                );
